...
Both sources provide raw match results, not pre-computed tables. Every finishing position is derived using the same function ([`fetch_standings.py`](https://github.com/kpolimis/epl-standings/blob/main/fetch_standings.py)):
...
The raw `pos` array isn't suitable input for a clustering algorithm directly — it's a ragged time series with gaps. Instead, [`extract_features()`](https://github.com/kpolimis/epl-standings/blob/main/cluster_trajectories.py) in `cluster_trajectories.py` extracts six scalar features that summarize the shape of each club's EPL career:
...
Here are the four clusters, sorted by average seasons played. The full reporting loop is in [`cluster_trajectories.py`](https://github.com/kpolimis/epl-standings/blob/main/cluster_trajectories.py) — run `python cluster_trajectories.py` to reproduce this output:
...
The EPL tourists and the yo-yo regulars have similar `first_season` averages (3.5 vs 4.3), meaning both groups were present early. What separates them is `stints` (1.62 vs 4.19) and `currently_in` (0% vs 50%). The EPL tourists mostly had one sustained run and then dropped out permanently. The yo-yo clubs kept coming back.
...
The four groups separate visually in a way that validates the clustering: the permanent fixtures run wall-to-wall; the yo-yo regulars have interrupted lines that resume; the modern entrants cluster in the right half of the chart; the EPL tourists have lines that end and don't come back.
